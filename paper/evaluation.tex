...
In this section we will demonstrate the validity of the model and our experimental interpretation of service times. First, we show a baseline case for a topology with one end-node in Figure~\ref{fig:1node}. In this scenario, the channel does not saturate and the service times are always small, rendering wait times to be dominated by sleep times. Therefore, the different in workloads is not observed and all plots overlap with the model. Now we consider the saturation case by comparing the model with a network with three end-nodes in Figure~\ref{fig:3nodes}.  
